...
pub fn current_capabilities() -> Vec<CapabilityArea> {
    vec![
        ...
        area("tls_quic", "TLS / QUIC boundaries", ObservationTier::InspectL1, CapabilityStage::Partial, VisibilityRisk::High, "L0 first-write copies parse TLS ClientHello SNI/ALPN and decrypt QUIC v1 Initial in user space (RFC 9001 salt+DCID keys, AES-128-GCM, CRYPTO reassembly) to recover the ClientHello SNI/ALPN with no hook and no process change; the first QUIC datagram per socket is copied up to 2048 bytes (8 datagrams per socket) so split client hellos still reassemble. --inspect-tls may be combined with --inspect-adapter binder_userspace. SSL_write/SSL_read attach on Conscrypt libssl.so, tethering stable_cronet_libssl.so, and app-private boringssl forks with the export (libhssl, libttboringssl) when exported; ELF64 is preferred and ELF32 is skipped when a 64-bit libssl with SSL_write is mapped (this arm64 GKI returns ENOTSUP for AArch32 uprobes). QUIC STREAM/HTTP/3 application bodies, Handshake and 1-RTT packets stay encrypted; draft versions other than v1 are not derived. dump tls_stacks classifies Conscrypt/Cronet/Flutter/mbedTLS/wolfSSL/GmSSL/TASSL/app libssl. Flutter Dart TLS, Cronet without exported SSL_write, and WebView/Chromium are not copied. App `libhssl` attaches exported `sslWrite`/`sslRead` (not OpenSSL `SSL_write`)."),
