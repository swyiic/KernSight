...
static __always_inline ksight_u16 ksight_port_from_sockaddr(const ksight_u8 *address,
                                                            ksight_u16 family)
{
    if (family != KSIGHT_AF_INET && family != KSIGHT_AF_INET6)
        return 0;
    return (ksight_u16)(((ksight_u16)address[2] << 8) | address[3]);
}

static __always_inline void ksight_note_dns_socket(ksight_u32 tgid, ksight_s32 fd,
                                                   ksight_u16 family,
                                                   const ksight_u8 *address)
{
    ksight_u8 tracked = 1;
    ksight_u64 key;

    if (ksight_port_from_sockaddr(address, family) != 53)
        return;
    key = ksight_socket_key(tgid, fd);
    ksight_bpf_map_update_elem(&dns_sockets, &key, &tracked, 0);
}
...
int ksight_network_connect_exit(struct ksight_raw_sys_exit *context)
{
    ...
    if (context->result >= 0 || context->result == -115) {
        ...
        ksight_note_dns_socket((ksight_u32)(pid_tgid >> 32), pending->fd,
                               pending->address_family, pending->address);
    }
    return 0;
}
...
#include "ksight_network_io.bpf.h"
#include "ksight_network_dns.bpf.h"
#include "ksight_network_handshake.bpf.h"
