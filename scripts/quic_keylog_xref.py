#!/usr/bin/env python3
"""ADRP+ADD xref scan for BoringSSL keylog labels in stripped QUIC/TLS SOs.

Works on libtquic.so / libxquic.so / libflutter.so (static BoringSSL). Finds
candidate ssl_log_secret as common BL after label materialization (typically x1).

IMPORTANT: loose multi-BL windows can pick wrappers. Prefer confirming with
symbols.zip / unstripped SO that shares the SAME GNU build-id (see Flutter
3.27.1: loose xref picked 0x9e7114; symbols same build-id gave true 0x6e4a78).
Crash-safe: print candidates only; promote to stack_rules only with build-id pin
+ verified evidence. Never invent offsets for unmatched builds.

Usage (when Mac brings forensics SO):
  python3 quic_keylog_xref.py /path/to/libtquic.so
"""
